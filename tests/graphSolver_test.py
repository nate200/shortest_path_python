...
@pytest.mark.parametrize("startNode, goalNode", [("nonExistingNode", "I"), ("A", "nonExistingNode")])
def test_start_node_not_exist(startNode, goalNode):
    filename = "t1.csv"
    with pytest.raises(Exception) as exc_info:
        graphSolver.solve_shortest_path_graph(filename, startNode, goalNode)
...
def test_csv_graph():
    ...
    actual = graphSolver.get_graph_from_csv("t1.csv")
...
def test_unreachable():
    ...
    actual = graphSolver.solve_shortest_path_graph(filename, startNode, goalNode)

    assert actual == None


@pytest.mark.parametrize("filename, startNode, goalNode, expected_path, expected_cost", [
    ("t1.csv", "A", "B", ["A", "B"], 5),
...
def test_shortest_path(filename, startNode, goalNode, expected_path, expected_cost):
    actual = graphSolver.solve_shortest_path_graph(filename, startNode, goalNode)
